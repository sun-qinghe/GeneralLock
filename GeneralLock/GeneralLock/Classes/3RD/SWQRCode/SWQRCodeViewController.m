...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    [self.navigationController.navigationBar setBackgroundImage:[UIImage imageWithColor:kSchemeColor] forBarMetrics:UIBarMetricsDefault];
...
- (void)_setupUI {
    ...
    Configuration *config = [Configuration mj_objectWithKeyValues:kUserDefaultObjectForKey(@"configuration")];
    NSLog(@">>>>>>>>>>%@",config.language);
    
    self.view.backgroundColor = [UIColor blackColor];
    UIBarButtonItem *albumItem = [[UIBarButtonItem alloc]initWithTitle:[config.language isEqualToString:@"EN"] ? @"Album" : @"相册" style:UIBarButtonItemStylePlain target:self action:@selector(showAlbum)];
    [albumItem setTintColor:UIColorWhite];
...
- (void)sw_handleWithValue:(NSString *)value {
    ...
    [self.sendResult sendNext:value];
...
-(RACSubject *)sendResult{
    if (!_sendResult) {
        _sendResult = [RACSubject subject];
    }
    return _sendResult;
}
