...
#define Scanner_Y (self.frame.size.height - Scanner_Width)/2 /** 扫描器初始y值 */
...
- (void)sw_setFlashlightOn:(BOOL)on {
    ...
    if ([self.configuration.language isEqualToString:@"EN"]) {
        self.flashlightLab.text = on ? @"Touch to flash off":@"Touch to light up";
    } else {
...
- (UIImageView *)scannerLine {
    if (!_scannerLine) {
        ...
        _scannerLine.image = UIImageNamed(@"ScannerLine");
    }
    return _scannerLine;
}
...
- (UILabel *)tipLab {
    if (!_tipLab) {
        ...
        if ([self.configuration.language isEqualToString:@"EN"]) {
            _tipLab.text = @"Please Scan The QRCode/BarCode";
        } else {
...
- (UIButton *)flashlightBtn {
    if (!_flashlightBtn) {
        ...
        [_flashlightBtn setBackgroundImage:UIImageNamed(@"Flashlight_Off") forState:UIControlStateNormal];
        [_flashlightBtn setBackgroundImage:UIImageNamed(@"Flashlight_On") forState:UIControlStateSelected];
    }
...
- (UILabel *)flashlightLab {
    if (!_flashlightLab) {
        ...
        if ([self.configuration.language isEqualToString:@"EN"]) {
            _flashlightLab.text = @"Touch to light up";
        } else {
